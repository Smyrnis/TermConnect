...
fn panel_with_file(name: &str, size: u64) -> PanelView {
...
fn renders_entry_names() {
    let panel = panel_with_file("readme.txt", 7);
...
fn wide_panel_shows_size_and_permissions_columns() {
    let panel = panel_with_file("readme.txt", 5);
...
fn narrow_panel_hides_size_and_permissions_columns() {
    let panel = panel_with_file("a.txt", 123_456);
