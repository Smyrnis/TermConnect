...
impl App {
    ...
    pub(super) fn disconnect_selected(&mut self) {
        ...
        let affected = self.transfers.fail_queued_for_session(id, "session disconnected")
            + self.cancel_session_transfers(id)
            + self.drop_conflict_reviews(|review| review.session_id == id);
        if affected > 0 {
