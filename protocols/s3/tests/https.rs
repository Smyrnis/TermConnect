...
use support::{BUCKET, Options, SECRET};

fn https_target(port: u16) -> Target {
    support::with_option(support::target(port, Some(BUCKET), Some(SECRET)), "security", "https")
}
...
async fn a_changed_certificate_is_refused_without_asking() {
    ...
    let error = S3::new(store).connect(&https_target(new_server.port), &mut prompter).await.err().unwrap();
