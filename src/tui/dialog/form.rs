...
impl FormDialog {
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> FormOutcome {
        match key.code {
            ...
            KeyCode::Enter => {
                FormOutcome::Submitted(self.fields.iter().map(|f| f.value.clone()).collect())
            }
            KeyCode::Tab => {
...
pub fn render_form(frame: &mut Frame, area: Rect, dialog: &FormDialog) {
    ...
    lines.extend(
        dialog
            ...
            .iter()
            .enumerate()
            .map(|(i, field)| field_line(field, i == dialog.focused)),
    );
    if let Some(error) = &dialog.error {
...
#[cfg(test)]
#[path = "../../../tests/tui/dialog/form_test.rs"]
mod tests;
