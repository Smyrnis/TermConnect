...
fn upload(name: &str, conflict: Option<bool>) -> PlannedFile {
    PlannedFile {
        ...
        remote_path: format!("/remote/dest/{name}"),
...
fn plan(files: Vec<PlannedFile>) -> DirectoryPlan {
    let names: HashSet<String> =
        files.iter().map(|file| file.remote_path.rsplit('/').next().unwrap().to_string()).collect();
...
fn rename_rewrites_the_local_path_for_downloads() {
    let file = PlannedFile {
        local_path: PathBuf::from("/local/dest/a.txt"),
...
fn rename_rewrites_the_remote_path_and_display_name_of_a_nested_upload() {
    let file = PlannedFile {
        local_path: PathBuf::from("/local/photos/sub/a.txt"),
        remote_path: "/remote/dest/photos/sub/a.txt".to_string(),
...
fn a_folder_blocks_resume() {
    let resolved = resolve(plan(vec![with(Some(true), true)]), &[Resolution::Resume], Direction::Upload);
...
fn rename_on_a_partial_only_file_resumes() {
    let files = resolve(plan(vec![with(None, true)]), &[Resolution::Rename], Direction::Upload).files;

    assert_eq!(files[0].remote_path, "/remote/dest/a.iso");
    assert!(files[0].resume);
}

#[test]
fn skip_on_a_partial_queues_nothing() {
    let resolved = resolve(plan(vec![with(None, true)]), &[Resolution::Skip], Direction::Upload);
